...
set.seed(39592)
fold1 <- createDataPartition(pca_training_preds$y, p=0.8, list = F)
trainfold1 <- pca_training_preds[fold1,]
testfold1 <- pca_training_preds[-fold1,]
mod_rf_fold1 <- randomForest(y~.,data = trainfold1)
pred_fold1 <- predict(mod_rf_fold1, testfold1)
print(confusionMatrix(pred_fold1, testfold1$y)$overall[1])

set.seed(40639)
fold2 <- createDataPartition(pca_training_preds$y, p=0.8, list = F)
trainfold2 <- pca_training_preds[fold2,]
testfold2 <- pca_training_preds[-fold2,]
mod_rf_fold2 <- randomForest(y~.,data = trainfold2)
pred_fold2 <- predict(mod_rf_fold2, testfold2)
print(confusionMatrix(pred_fold2, testfold2$y)$overall[1])

set.seed(94823)
fold3 <- createDataPartition(pca_training_preds$y, p=0.8, list = F)
trainfold3 <- pca_training_preds[fold3,]
testfold3 <- pca_training_preds[-fold3,]
mod_rf_fold3 <- randomForest(y~.,data = trainfold3)
pred_fold3 <- predict(mod_rf_fold3, testfold3)
print(confusionMatrix(pred_fold3, testfold3$y)$overall[1])
```
...
for (thresh in threshs){
  ...
  #print(confusionMatrix(pred_plot, testfold$y)$overall[[1]])
...
#preObj.test <- preProcess(test_num, method = "pca", thresh=0.75)
